...
object Cancellation {
    ...
    suspend fun developer(index: Int) {
        ...
        while(true) {
            delay(500)
            LOGGER.info("[dev $index] I'm still working on a feature.")
        }
    }
    ...
    suspend fun ceo(employee: Job) {
        ...
        employee.cancel() // job will get cancelled at the next suspension point
        LOGGER.info("[ceo] I have fired the developer.")
        employee.invokeOnCompletion { cause ->
            if (cause is CancellationException) {
                LOGGER.info("The CEO terminated the employee contract.")
            }
    ...
    // IF A COROUTINE DOES NOT HAVE A SUSPENSION POINT, IT  IS _____NOT_____ CANCELABLE
    ...
    suspend fun startUp() {
        LOGGER.info("9AM, beautiful day to change the world!")
        coroutineScope {
            val goodDeveloper  = launch { developerWithTry(3) }
            val lazyDeveloper = launch { developerWithTry(42) }
            launch { ceo(lazyDeveloper) }
        }
    ...
    suspend fun developerAtWork(index: Int) {
        Laptop("Vishnu's macbook").use {laptop ->
